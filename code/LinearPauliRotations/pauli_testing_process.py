...
version_dict = {
    'v1': LinearPauliRotations_defect1, 
    'v2': LinearPauliRotations_defect2,
    'v3': LinearPauliRotations_defect3,
    'v4': LinearPauliRotations_defect4,
    'v5': LinearPauliRotations_defect5
}
...
def testing_with_doss(
    ...
    for test_order in range(len(df)): 
        ...
        # Form the partitions
        partition_list = [0, 1]        # 0: q[0]-q[n-1], 1: q[n]
        partition_order = np.random.choice(
            partition_list, 
            size=len(partition_list), 
            replace=False
        )
        for partition in partition_order:
            if partition == 0:
                qc_initial = QuantumCircuit(total_qubits, input_qubits)
                for index, val in enumerate(initial_state[::-1]):
                    if val == 1:
                        qc_initial.x(index)
                ...
                qc.append(qc_test, qc.qubits)
                qc.measure(qc.qubits[:input_qubits], qc.clbits)
                
                # Use the backend to execute the quantum circuit
                dict_counts = measurement_based_backend(qc, shots, backend)
                
                # Execute the WOO method to determine the test result
                test_result = woo_execution(dict_counts, number, woo_toler_err)
                ...
                # Prepare the specification state
                beta, theta = program_specification_angles(
                    input_qubits, 
                    input_qubits, 
                    number, 
                ...
                # Perform the swap test 
                qc.h(-1)
                qc.cswap(-1, input_qubits, total_qubits)
                qc.h(-1)
                qc.measure(qc.qubits[-1],qc.clbits)
